      ...
      if( CheckAndSetState( psCmdSeqHandler, CMD_SEQ_WAITING_FOR_RESP, CMD_SEQ_BUSY ) )
      {
         if( psCmdSeqHandler->pasCmdSeq[ psCmdSeqHandler->bCurrSeqIndex ].pnRespHandler != NULL )
         {
            ...
            */
            ...
            ABCC_PORT_DebugPrint( ( "CmdSeq(%p)->%s()\n",
               psCmdSeqHandler->pasCmdSeq,
            ...
            {
#if ABCC_CFG_DEBUG_CMD_SEQ_ENABLED
               ABCC_PORT_DebugPrint( ( "CmdSeq(%p)->Aborted\n",
                                       psCmdSeqHandler->pasCmdSeq ) );
#endif
               /*
               ** Loop until end of sequence and skip done callback.
               */
               while( psCmdSeqHandler->pasCmdSeq[ ++psCmdSeqHandler->bCurrSeqIndex ].pnCmdHandler != NULL )
               {
               };
               psCmdSeqHandler->pnSeqDone = NULL;
            }
            ...
            {
               ABCC_PORT_DebugPrint( ( "CmdSeq(%p)->Executing same sequence step again\n",
                                       psCmdSeqHandler->pasCmdSeq ) );
            }
#endif
         }
         ...
         {
#if ABCC_CFG_DEBUG_CMD_SEQ_ENABLED
            ABCC_PORT_DebugPrint( ( "CmdSeq(%p)->Response received\n",
                                    psCmdSeqHandler->pasCmdSeq ) );
#endif
            /*
...
static BOOL ExecCmdSequence( CmdSeqHandlerType* psCmdSeqHandler, ABP_MsgType* psMsg )
{
   ...
   if( psMsg != NULL )
   {
      ...
      while( ( psCmdSeq->pnCmdHandler != NULL ) && !fCmdBufferConsumed )
      {
         ...
         {
#if ABCC_CFG_DEBUG_CMD_SEQ_ENABLED
            ABCC_PORT_DebugPrint( ( "CmdSeq(%p)->Command not sent, jump to next sequence step\n",
                                    psCmdSeqHandler->pasCmdSeq ) );
#endif
            /*
...
#if ABCC_CFG_DEBUG_CMD_SEQ_ENABLED
            ABCC_PORT_DebugPrint( ( "CmdSeq(%p)->Done\n",
                                    psCmdSeqHandler->pasCmdSeq ) );
#endif
