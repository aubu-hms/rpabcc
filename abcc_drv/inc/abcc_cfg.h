...
** "abcc_serial_drv.c" for each serial telegram cycle.
**
...
** (ABCC_CFG_INT_ENABLED = FALSE). Presently this is used only to detect the
** startup interrupt.
**
** If TRUE the user must implement the ABCC_SYS_IsAbccInterruptActive() function
** in the system adaption layer.
**------------------------------------------------------------------------------
*/
...
** connector). If TRUE the user must implement the following functions in the
...
/*------------------------------------------------------------------------------
** #define ABCC_CFG_ADI_TRANS_SET_CALLBACK (BOOL - TRUE/FALSE )
**
** Defined in abcc_drv_cfg.h.
**
...
** enabled by ABCC_CFG_ADI_GET_SET_CALLBACK isn't called from the application
** data object command handler when this define is enabled and
** pnSetAdiValueTransparent isn't NULL. This define will affect the
** AD_AdiEntryType (abcc_ad_if.h) used for defining the user ADI:s.
**------------------------------------------------------------------------------
*/
...
/*------------------------------------------------------------------------------
** #define ABCC_CFG_DOUBLE_ADI_SUPPORT
**
** Defined in abcc_drv_cfg.h.
**
...
/*------------------------------------------------------------------------------
** #define ABCC_CFG_DEBUG_HEXDUMP_MSG          (BOOL - TRUE/FALSE )
** #define ABCC_CFG_DEBUG_HEXDUMP_SPI          (BOOL - TRUE/FALSE )
** #define ABCC_CFG_DEBUG_HEXDUMP_UART         (BOOL - TRUE/FALSE )
**
** Defined in abcc_drv_cfg.h.
**
...
** add a printout when the ABCCs state changes. ABCC_PORT_DebugPrint() in
** "abcc_sw_port.h" is used to print this information and must therefore be
** ported for this to work.
**------------------------------------------------------------------------------
*/

/*------------------------------------------------------------------------------
** #define ABCC_CFG_DEBUG_CRC_ERROR_CNT        (BOOL - TRUE/FALSE )
**
** Defined in abcc_drv_cfg.h.
**
** If TRUE an UINT16 counter for CRC errors for the SPI and UART application
...
** via an extern declaration in "abcc_debug_err.h".
**------------------------------------------------------------------------------
*/
...
** (ABCC_CFG_POLL_ABCC_IRQ_PIN) this value will be used as a timeout. If the
** interrupt signal from the ABCC has not been seen within this time it is
** assumed that the ABCC is not present or not responding.
**
...
/*------------------------------------------------------------------------------
** #define ABCC_CFG_DRV_CMD_SEQ_ENABLE        ( BOOL - TRUE/FALSE )
**
** Defined in abcc_drv_cfg.h.
